module CompMultiplier (
    ...
    Interface M1 (
        ...
        .multiplier(multiplier),.multiplicand(multiplicand),.product_in(product_w),.valid_in(valid_w),    
        .start_out(start_w),.preload_out(preload_w),
        .multiplier_out(multiplier_w),.multiplicand_out(multiplicand_w),.product(product),.valid(valid)           
    );
    Multiplicand M2 (
